...
function Provider:__init(full)
  ...
  -- download dataset
--   if not paths.dirp('cifar-10-batches-t7_generated') then
...
--   end
  ...
  for i = 0,4 do
     local subset = torch.load('generated_data/data_batch_' .. (i+1) .. '.t7')
     trainData.data[{ {i*10000+1, (i+1)*10000} }] = subset.data
     trainData.labels[{ {i*10000+1, (i+1)*10000} }] = subset.labels
  end
  trainData.labels = trainData.labels + 1

  local subset = torch.load('original_data/data_batch_1.t7')
  self.testData = {
     data = subset.data:double(),
